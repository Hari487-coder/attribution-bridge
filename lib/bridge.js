...
async function recreateInBroker(sourceContact, sourceLoc, sourceToken, broker, settings, marker) {
  ...
  const tags = [
    ...new Set(
      [...(sourceContact.tags ?? []), settings.bridgeTag, marker ? settings.verifiedTag || "assistable-verified" : null].filter(
        Boolean
      )
    ),
  ];
  ...
  for (const id of contactIds) {
    try {
      ...
      const body = buildCreateBody(full, {
        tags: [
          ...new Set(
            [...(full.tags ?? []), config.settings.bridgeTag, marker ? config.settings.verifiedTag || "assistable-verified" : null].filter(
              Boolean
            )
          ),
        ],
