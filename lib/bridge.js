...
async function recreateInBroker(sourceContact, sourceLoc, sourceToken, broker, settings, marker) {
  ...
  if (!marker || !verify.verifyMarker(marker)) {
    return {
      ...
      reason:
        "refusing to create a broker contact without a valid signed verification record — attribution evidence (signed under this workspace's key) is required for every bridged lead.",
    };
  }
  ...
  // Use the RAW source phone so verify's international-aware registryKey matches
  // how the opt-out was stored; `phone` above is the NANP-only dialer normalizer.
  if (verify.isWithdrawn(sourceContact.phone)) {
    return {
      ok: false,
      refused: true,
  ...
  // Suppression backstop re-check before any mutation (a number may have been
  // added to the RND/litigator/DNC list after the gate ran).
  if (verify.isSuppressed(sourceContact.phone)) {
    return {
      ok: false,
      refused: true,
      step: "suppressed",
      reason: "number is on the suppression list (reassigned/litigator/complainer/internal DNC) — aborting bridge.",
    };
  }
  ...
  if (dupes.length > 0) {
    ...
    if (policy === "strip") {
      ...
      // on GHL dedupe. Send null (not ""): GHL v2 validates email and rejects an
      // empty string with "email must be an email", whereas null clears it. Read
      // back to confirm the clear actually took — some GHL setups ignore the
      // value; if the phone still matches, abort loudly rather than let the
      // create silently collide.
      for (const d of dupes) {
        await ghl.updateContact(d.id, { phone: null, email: null }, broker.token);
        stripped.push(d.id);
  ...
  // Stamp the verifiable consent record as a contact note (best-effort audit
  // trail — a note failure must not fail the bridge). The record points back to
  // the master so Assistable/an auditor can independently re-verify the opt-in.
  let markerNoteWritten = false;
  if (marker && createdId) {
    try {
      const consent = verify.consentEvidenceFor(marker.verifiedPhone);
      await ghl.createContactNote(createdId, verify.consentNote(consent, marker), broker.token);
 ...
 */
async function distributeLead({ contactId, brokerKey, nationalDnc = null }, config) {
  ...
  return serialize(`${brokerKey}::${contactId}`, async () => {
    ...
    // Verify the MASTER's attribution evidence before bridging. The master
    // record's attribution / createdBy are set by GHL from the source, not by
    // the standard contact-update API — this confirms the fields satisfy policy,
    // not that consent is genuine (the master is the trust anchor; see verify.js).
    // A lead with no acceptable attribution evidence is NEVER created in a
    // broker — this is unconditional and cannot be turned off.
    ...
    // National DNC status is NOT looked up here — that is Assistable's compliance
    // engine to own, and duplicating it would drift when the platform changes
    // provider/exemption/state logic. We only RECORD it (for the audit trail) when
    // an authoritative source passes it in (e.g. the webhook body); otherwise null.
    ...
    verify.registerVerification({
      ...
      masterContactId: contactId,
      masterLocationId: config.master.locationId,
...
async function migrateRun(brokerKey, contactIds, config, { dryRun = true, overrideTags = null } = {}) {
  ...
  for (const id of contactIds) {
    try {
      ...
      // Verify-first for backlog. The ONLY acceptable evidence is a LIVE,
      // currently-opted-in contact in the MASTER account for this phone — the
      // attribution fields GHL sets from the source. (The master is the trust
      // anchor; the bridge can't independently prove that evidence is genuine.)
      // We deliberately do NOT trust a stored registry "active" status here:
      // app-local state can be reset or restored from an uploaded bundle, so it
      // must never be the thing that authorizes a call. Re-verify against the
      // master on every run; if the evidence isn't present live, refuse (fail closed).
      let marker = null;
      if (config.master?.token && config.master?.locationId) {
        ...
        results.push({
          ...
          reason:
            config.master?.token && config.master?.locationId
              ? "no currently opted-in master contact for this phone — cannot confirm attribution evidence live; bridge it from the master first."
              : "master account not configured — backlog migration re-verifies each number's attribution evidence against the master.",
        });
        continue;
      }
      ...
      // re-migrating a lead doesn't restart its broker automations. Otherwise the
      // migrateTagMode setting decides how many of the broker contact's OWN tags to
      // keep ("all" by default = unchanged; "list"/"none" trim them so the recreate
      // doesn't re-fire the broker's tag-triggered workflows). The app's bridge/
      // verified tags and alwaysAddTags always apply on top; opt-out/DND are fields,
      // so trimming tags never weakens the DNC gate.
      const migTags = Array.isArray(overrideTags)
        ...
        : selectCarryTags(
            ...
            {
              ...config.settings,
              tagCopyMode: config.settings.migrateTagMode || "all",
              tagCopyList: config.settings.migrateTagList || [],
            },
            !!marker
          );
      ...
      // during the searches above (the stale-marker race). Use the RAW phone so
      // verify's international-aware key matches how the opt-out was stored.
      if (verify.isWithdrawn(full.phone)) {
        results.push({ id, action: "skip", reason: "number was opted out during processing — opt-out always wins." });
        continue;
      }
      if (verify.isSuppressed(full.phone)) {
        results.push({ id, action: "skip", reason: "number is on the suppression list (reassigned/litigator/complainer/internal DNC) — not bridged." });
        continue;
      }
      ...
      if (policy === "strip") {
        ...
        // attributed contact won't collide on GHL dedupe. Send null (not ""):
        // GHL v2 rejects email:"" with "email must be an email"; null clears it.
        // Read back to confirm.
        await ghl.updateContact(id, { phone: null, email: null }, broker.token);
      ...
      if (marker) {
        try {
          const consent = verify.consentEvidenceFor(marker.verifiedPhone);
          await ghl.createContactNote(created.id, verify.consentNote(consent, marker), broker.token);
        } catch {
...
/**
 * Propagate an opt-out to already-bridged copies. The registry opt-out blocks
 * FUTURE bridges, but a copy created BEFORE the opt-out keeps its INTEGRATION
 * stamp and stays callable. This finds the number across every configured broker
 * subaccount and sets it Do-Not-Call, so the dialer stops immediately. Best
 * effort per broker and per contact — never throws; returns a summary.
 */
async function suppressAcrossBrokers(phone, config) {
  const summary = { checked: 0, suppressed: [], errors: [] };
  const brokers = (config && config.brokers) || {};
  for (const [key, b] of Object.entries(brokers)) {
    if (!b || !b.locationId || !b.token) continue;
    summary.checked += 1;
    try {
      const matches = await ghl.searchByPhone(b.locationId, phone, b.token);
      for (const c of matches) {
        try {
          await ghl.updateContact(c.id, { dnd: true }, b.token);
          summary.suppressed.push({ broker: key, contactId: c.id });
        } catch (e) {
          summary.errors.push({ broker: key, contactId: c.id, error: e.message });
        }
      }
    } catch (e) {
      summary.errors.push({ broker: key, error: e.message });
    }
  }
  return summary;
}
