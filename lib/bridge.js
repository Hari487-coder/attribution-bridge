...
const { resolveDncAttributionEvidence, normalizePhone, isOnExternalDnc } = require("./compliance");
...
async function distributeLead({ contactId, brokerKey }, config) {
  ...
  return serialize(`${brokerKey}::${contactId}`, async () => {
    ...
    // Jorden's rule 2, documented: the internal-DNC layer (opt-out + suppression)
    // already ran absolutely in assessMaster; if enabled, also record the NATIONAL
    // registry status so the audit trail shows we relied on the attribution
    // exemption for a DNC-listed number. Best-effort / fail-open — never blocks.
    let nationalDnc = null;
    if (config.settings?.nationalDncCheck) {
      try {
        nationalDnc = (await isOnExternalDnc(assessment.phone)).registered;
      } catch {
        nationalDnc = null;
      }
    }
    ...
    try {
      ...
      if (config.master?.token && config.master?.locationId) {
        ...
        if (opted) {
          ...
          if (config.settings?.nationalDncCheck) {
            try {
              nationalDnc = (await isOnExternalDnc(phone)).registered;
            } catch {
              nationalDnc = null;
            }
          }
