...
async function recreateInBroker(sourceContact, sourceLoc, sourceToken, broker, settings, marker) {
  ...
  // Create WITHOUT tags in the body, then add them via the dedicated add-tags
  // endpoint below. Tags set at create time often don't emit a "tag added" event,
  // so the broker's tag-triggered automations never fire; adding them afterward
  // does emit that event.
  const body = buildCreateBody(sourceContact, { customFields: cfResult.mapped });
  ...
  // Apply tags via the dedicated add-tags endpoint so the broker's tag-triggered
  // automations actually fire. Best-effort: a tag failure must not fail the bridge
  // (the contact is already created with the INTEGRATION stamp), but it is surfaced
  // so a token-scope issue is visible.
  let tagsApplied = { ok: true, tags };
  if (createdId && tags.length) {
    try {
      await ghl.addTags(createdId, tags, broker.token);
    } catch (err) {
      tagsApplied = { ok: false, error: err.message, tags };
    }
  }
...
async function migrateRun(brokerKey, contactIds, config, { dryRun = true } = {}) {
  ...
  for (const id of contactIds) {
    try {
      ...
      // via the add-tags endpoint after create (like recreateInBroker) so the
      // broker's tag-triggered automations fire.
