...
async function migrateScan(
  brokerKey,
  config,
  { maxPages = 10, pageSize = 100, includeTags = [], excludeTags = [], includeMode = "any", masterIncludeTags = [], masterExcludeTags = [], masterIncludeMode = "any", masterMaxPages = 20 } = {}
) {
  const broker = config.brokers[brokerKey];
  ...
  if (useMaster && candidates.length) {
    ...
    for (const cand of candidates) {
      ...
      if (contactPassesTagFilter(mtags || [], { includeTags: masterIncludeTags, excludeTags: masterExcludeTags, includeMode: masterIncludeMode })) {
...
async function migrateRun(brokerKey, contactIds, config, { dryRun = true } = {}) {
  ...
  for (const id of contactIds) {
    try {
      ...
      // Preserve ALL of the broker contact's own tags (mode "all") + any force-add
      // tags. Applied per tagApplyMode, exactly like recreateInBroker.
