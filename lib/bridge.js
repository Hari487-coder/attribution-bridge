 ...
 *
 * GHL's list endpoint returns a slimmed contact that OMITS attributionSource /
 * lastAttributionSource / createdBy — so eligibility cannot be judged from it
 * (every contact would look un-attributed). We list to get the id set, then
 * fetch each full contact (concurrency-limited) to assess accurately.
 */
async function masterScan(config, { maxPages = 10, pageSize = 100, concurrency = 4 } = {}) {
  if (!config.master?.token || !config.master?.locationId) {
    return { ok: false, error: "Master location/token not configured." };
  }
  const listed = [];
