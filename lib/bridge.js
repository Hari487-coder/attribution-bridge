...
function selectCarryTags(sourceTags, settings = {}, hasMarker = false) {
  ...
  const seen = new Set();
  const out = [];
  for (const t of [...carried, ...appTags]) {
    if (!t) continue;
 ...
 * fail CastigliaAI's evidence check (candidates for recreate). An optional
 * broker-side tag filter narrows the list (e.g. exclude already-"Sold" contacts).
 */
async function migrateScan(brokerKey, config, { maxPages = 10, pageSize = 100, includeTags = [], excludeTags = [] } = {}) {
  const broker = config.brokers[brokerKey];
  ...
  appendLog({ kind: "migrate-scan", brokerKey, scanned, candidates: candidates.length, hiddenByTag });
  return { ok: true, brokerKey, scanned, candidates, hiddenByTag, truncated: scanned >= maxPages * pageSize };
}
