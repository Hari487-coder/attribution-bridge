...
function assessMaster(masterContact) {
  ...
  // untrusted origin is refused. Empty allowlist = accept any genuine attribution.
...
function explain(masterContact) {
  ...
  if (phone) {
    ...
    const evidence = resolveStrictAttributionEvidence(masterContact);
    add("attribution", !!evidence, evidence ? `genuine attribution present (${evidence})` : "no genuine marketing attribution (a bare INTEGRATION stamp is not accepted)");
    const trusted = loadConfig().settings?.trustedAttributionSources;
    if (Array.isArray(trusted) && trusted.length > 0) {
