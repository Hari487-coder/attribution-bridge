 ...
 *
 *   1. Verify the MASTER contact's attribution evidence (populated
 *      attributionSource / lastAttributionSource, not-DND) against the source
 *      record. GHL sets these fields from the lead's session/source, not the
 *      standard contact-update API. NOTE: this confirms the fields SATISFY THE
 *      POLICY — it is not proof of genuine consent. Whoever controls the master
 *      GHL location can still manufacture attribution (see the trust boundary).
...
const { resolveStrictAttributionEvidence, attributionSnapshot, isCallDnd, isOnCustomDnc } = require("./compliance");
...
/**
 * Assess whether a MASTER contact's fields satisfy the attribution policy (safe to bridge).
...
function assessMaster(masterContact) {
  ...
  // Mirror the dialer's step 0: the platform hard-DNC list blocks unconditionally,
  // before everything. Never bridge a number the platform will never call.
  if (isOnCustomDnc(masterContact.phone)) {
    return {
      verified: false,
      phone,
      reason: "number is on the platform hard-DNC list — the dialer never calls it, so it is never bridged.",
    };
  }
  ...
  if (!evidence) {
    return {
      ...
      phone,
      reason:
        "master contact has no acceptable attribution evidence (no populated attributionSource / lastAttributionSource). A bare INTEGRATION stamp is NOT accepted — it is also what a cold-list import carries — so this lead is refused to keep cold lists off the dialer.",
    };
  }
  const source = attributionSnapshot(masterContact);
  // Optional defense-in-depth for the master-as-trust-anchor risk: if an allowlist
  // is configured, the attribution must come from a trusted source (its values
  // must contain an allowlisted token), so forged/junk attribution from an
  // untrusted origin is refused. Empty allowlist = accept any populated attribution.
  const trusted = loadConfig().settings?.trustedAttributionSources;
  if (Array.isArray(trusted) && trusted.length > 0) {
    const hay = source ? Object.values(source.values).join(" ").toLowerCase() : "";
    const matched = trusted.some((t) => t && hay.includes(String(t).trim().toLowerCase()));
    if (!matched) {
      return {
        verified: false,
        phone,
        reason: "attribution is not from a trusted source (not on the trusted-attribution allowlist) — refusing to bridge potentially forged/untrusted attribution.",
      };
    }
  }
  return { verified: true, phone, evidence, source };
}

/**
 * Non-short-circuit diagnosis for the "Explain" view: runs EVERY gate and returns
 * all pass/fail results (assessMaster returns only the first failure). Same
 * verdict as assessMaster — `verified` is true iff nothing failed.
 */
function explain(masterContact) {
  const checks = [];
  const add = (name, pass, detail) => checks.push({ name, pass, detail });
  if (!masterContact) {
    return { verified: false, phone: null, checks: [{ name: "contact", pass: false, detail: "master contact not found" }], reasons: ["master contact not found"] };
  }
  const phone = masterContact.phone ? registryKey(masterContact.phone) : null;
  add("phone", !!phone, phone ? `phone ${phone}` : "master contact has no phone");
  if (phone) {
    const onPlatformDnc = isOnCustomDnc(masterContact.phone);
    add("platform-dnc", !onPlatformDnc, onPlatformDnc ? "on the platform hard-DNC list" : "not on the platform hard-DNC list");
    const withdrawn = loadRegistry()[phone]?.status === "withdrawn";
    add("opt-out", !withdrawn, withdrawn ? "number was opted out (opt-out always wins)" : "not opted out");
    const suppressed = isSuppressed(phone);
    add("suppression", !suppressed, suppressed ? "on the suppression list (reassigned / litigator / complainer / internal DNC)" : "not on the suppression list");
    const dnd = isCallDnd(masterContact);
    add("dnd", !dnd, dnd ? "contact is set DND for calling" : "not DND");
    const maxAge = Number(loadConfig().settings?.maxConsentAgeDays || 0);
    if (maxAge > 0 && masterContact.dateAdded) {
      const ageDays = (Date.now() - new Date(masterContact.dateAdded).getTime()) / 86_400_000;
      const stale = Number.isFinite(ageDays) && ageDays > maxAge;
      add("recency", !stale, stale ? `master record ~${Math.round(ageDays)}d old (> ${maxAge}d) — stale consent` : `within the ${maxAge}d recency window`);
    }
    const evidence = resolveStrictAttributionEvidence(masterContact);
    add("attribution", !!evidence, evidence ? `attribution evidence present (${evidence})` : "no acceptable attribution evidence (a bare INTEGRATION stamp is not accepted)");
    const trusted = loadConfig().settings?.trustedAttributionSources;
    if (Array.isArray(trusted) && trusted.length > 0) {
      const src = attributionSnapshot(masterContact);
      const hay = src ? Object.values(src.values).join(" ").toLowerCase() : "";
      const matched = trusted.some((t) => t && hay.includes(String(t).trim().toLowerCase()));
      add("trusted-source", matched, matched ? "attribution is from a trusted source" : "attribution is not from a trusted source (not on the allowlist)");
    }
  }
  const reasons = checks.filter((c) => !c.pass).map((c) => c.detail);
  return { verified: reasons.length === 0, phone, checks, reasons };
}

/** Record (or refresh) a verification. Returns the stored record. */
function registerVerification({ phone, evidence, masterContactId, masterLocationId, workspaceLabel, source, nationalDnc }) {
  ...
  reg[n] = {
    ...
    // independently re-verifiable against the master (the source of record).
    source: source ?? null,
    masterContactId: masterContactId ?? null,
    masterLocationId: masterLocationId ?? null,
    workspaceLabel: workspaceLabel ?? null,
    // National-registry status at bridge time (null = not checked). When true, we
    // bridged under the attribution opt-in exemption (Jorden's rule 2) — recorded
    // for a defensible audit trail.
    nationalDnc: nationalDnc ?? null,
    verifiedAt,
    status: "active",
  ...
  saveRegistry(reg);
  appendLog({ kind: "verify-register", phone: n, evidence, nationalDnc: nationalDnc ?? null, masterContactId: masterContactId ?? null });
  return reg[n];
}
...
// carries the concrete attribution values AND a pointer to the source of record
// (the master location + contact), so a third party (Assistable's backend, a
// compliance auditor) can re-fetch the master and confirm the attribution itself,
...
// which is the trust anchor, so the evidence is only as good as the master.
const CONSENT_VERSION = 1;

/**
 * Deterministic, RECURSIVELY key-sorted JSON. NB: JSON.stringify(obj, arrayOfKeys)
 * is a property ALLOWLIST applied at every depth — it silently drops nested keys,
 * which would leave `source`/`master` out of the signature. This covers every
 * nested field so the signature actually protects the provenance.
 */
...
function signConsent(payload) {
  return crypto.createHmac("sha256", ensureSigningSecret()).update(canonicalize(payload)).digest("hex");
}
// Keyless SHA-256 of the canonical payload. This is a stable CONTENT ID for
// referencing/correlating a record — NOT tamper-proof on its own (anyone can
// recompute it after editing a field). The HMAC `sig` is the tamper-evidence.
function hashConsent(payload) {
...
function consentEvidenceFor(phone) {
  ...
  const payload = {
    v: CONSENT_VERSION,
    bridgeVersion: BRIDGE_VERSION, // fingerprint: which bridge build produced this
    phone: r.phone,
    ...
    nationalDnc: r.nationalDnc ?? null, // recorded only if an authoritative source supplied it
    verifiedAt: r.verifiedAt,
  };
  // hash = keyless content ID; sig = HMAC tamper-evidence. Both over the payload.
  return { ...payload, hash: hashConsent(payload), sig: signConsent(payload) };
}
...
function verifyConsent(record) {
  ...
  const { sig, hash, ...payload } = record;
  const a = Buffer.from(String(sig));
  const b = Buffer.from(signConsent(payload));
  return a.length === b.length && crypto.timingSafeEqual(a, b);
}
...
function consentNote(record, marker) {
  ...
  // Human line: sanitize values so a stray '<' or newline can't corrupt the note.
  const clean = (s) => String(s).replace(/[<>\r\n]+/g, " ").trim();
  const src = record.source?.values
    ? Object.entries(record.source.values).map(([k, v]) => `${clean(k)}=${clean(v)}`).join(", ")
    : "n/a";
  // Machine block: base64 of the exact signed record. base64's charset excludes
  // '<', so no attribution value can break the </consent> delimiter, and the
  // decoded bytes are byte-identical to what was signed (verifyConsent passes).
  const b64 = Buffer.from(JSON.stringify(record)).toString("base64");
  return (
    `${label} | evidence=${clean(record.evidence)} | source=${src} | ` +
    `master=${clean(record.master.locationId ?? "?")}/${clean(record.master.contactId ?? "?")} | at=${clean(record.verifiedAt)}\n` +
    `<consent b64="1">${b64}</consent>`
  );
}

/** Parse a consent record out of a bridged-contact note (see consentNote). */
function parseConsentNote(note) {
  const m = String(note || "").match(/<consent b64="1">([A-Za-z0-9+/=]+)<\/consent>/);
  if (!m) return null;
  try {
