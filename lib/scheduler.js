...
function loadState() {
  try {
    return JSON.parse(fs.readFileSync(tenant.schedulerPath(), "utf8"));
  } catch {
    return { lastDigestDate: null, lastErrorAlertIso: null };
  }
}
function saveState(s) {
  const p = tenant.schedulerPath();
...
/** Run one tick for EVERY tenant, each inside its own tenant context. */
async function tickAll() {
  for (const acct of tenant.listAccounts()) {
    if (!acct.tenantId || acct.disabled) continue;
    await tenant.runInTenant(acct.tenantId, () => tick().catch(() => {}));
  }
}
...
function start() {
  ...
  const safeTick = () => tickAll().catch(() => {});
  setTimeout(safeTick, 20_000);
  handle = setInterval(safeTick, TICK_MS);
  if (handle.unref) handle.unref();
}

module.exports = { start, tick, tickAll, sendDigest };
