/**
 * compliance.js — faithful model of Assistable's DNC gate behavior, used to
 ...
 *   - Assistable's internal per-subaccount DNC list (opt-outs recorded in their DB)
...
async function simulateGate(contact, phone, { fetchImpl } = {}) {
  ...
  const checks = {};
  const caveats = [
    "Assistable's internal per-subaccount DNC list (recorded opt-outs) is not visible from outside — a prior 'stop calling me' would still block.",
  ...
  if (checks.platformDnc) {
    return {
      ...
      reason: "platform_dnc",
      detail:
        "Number is on Assistable's hardcoded platform DNC list — blocks unconditionally, before DND / attribution / bypass.",
      evidence: null,
      normalized,
      checks,
      caveats,
  ...
  if (evidence) {
    return {
      ...
      detail: `Opt-in evidence "${evidence}" present — Assistable skips internal + national DNC for this contact.`,
      evidence,
      normalized,
      checks,
      caveats,
  ...
  if (external.registered) {
    return {
      wouldBlock: true,
      reason: "national_dnc",
      detail: "No attribution evidence and the number is on the national DNC registry — Assistable blocks with 403 'Number on national DNC list'.",
      evidence: null,
      normalized,
      checks,
      caveats,
  ...
  return {
    ...
    detail: external.error
      ? `No attribution evidence; national DNC lookup errored (${external.error}) — Assistable fails open, call would proceed (unless internally DNC'd).`
