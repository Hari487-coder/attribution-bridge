...
// Mirror GHL's server-side contact validation in mock mode so tests catch what
// the real API rejects. GHL guards `email` with an IsOptional()+IsEmail() rule:
// an absent key, null, or undefined CLEARS the field (allowed), but an empty
// string "" — or any non-address string — fails with "email must be an email".
// Phone has no format gate, so ""/null both simply clear it. Without this, the
// mock's bare Object.assign accepts email:"" and hides real strip failures.
function assertValidEmailField(fields) {
  if (!fields || !Object.prototype.hasOwnProperty.call(fields, "email")) return;
  const email = fields.email;
  if (email == null) return; // null/undefined → clears the field, allowed
  if (typeof email !== "string" || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    throw new GhlError("email must be an email", 422, null);
  }
}
...
function seedMocks() {
  ...
  // Master contact: Meta lead-form arrival — populated ad attribution + integration stamp.
  mockContact({
    id: "master_meta_1",
    locationId: "loc_master",
    phone: "+15551230001",
    attributionSource: { sessionSource: "Facebook", medium: "paid", utmSource: "fb_leadform" },
    createdBy: { source: "INTEGRATION", channel: "OAUTH", sourceId: "app_meta_sync" },
  });
  // Master contact: INTEGRATION-stamped but NO attribution — the shape a cold-list
  // API import takes. The strict bridge gate must REFUSE this even though the
  // dialer's own (looser) gate would allow it.
  mockContact({
    id: "master_intonly_1",
    locationId: "loc_master",
    firstName: "IntOnly",
    phone: "+15551230077",
    createdBy: { source: "INTEGRATION", channel: "API", sourceId: "bulk_import" },
  });
  ...
  mockContact({
    ...
    attributionSource: { utmSource: "ig", medium: "paid" },
    createdBy: { source: "INTEGRATION" },
  });
  mockContact({
  ...
  // Master-tag-filter pair: the broker copy does NOT carry the assignment tag,
  // but its MASTER (matched by phone) does — proves migrateScan can filter the
  // backlog by the MASTER's authoritative tags, not just the broker copy's.
  mockContact({
    id: "master_assign_1",
    locationId: "loc_master",
    firstName: "Assign",
    phone: "+15550777001",
    tags: ["valor assurance", "georgia"],
    attributionSource: { utmSource: "ig", medium: "paid" },
    createdBy: { source: "INTEGRATION" },
  });
  mockContact({
    id: "broker_assign_copy",
    locationId: "loc_broker_a",
    firstName: "AssignCopy",
    phone: "+15550777001",
    tags: ["fb lead"],
    createdBy: { source: "USER", channel: "COPY" },
  });
  // Backlog-migration duplicate-policy fixtures: no-evidence broker contacts, one
  // per policy (skip / strip / recreate), each on a unique phone with no sibling.
  for (const [id, phone] of [
    ["broker_pol_skip", "+15550911001"],
    ["broker_pol_strip", "+15550911002"],
    ["broker_pol_recreate", "+15550911003"],
    ["broker_pol_override", "+15550911004"],
  ]) {
    mockContact({ id, locationId: "loc_broker_a", firstName: "Pol", phone, email: `${id}@x.com`, tags: ["fb lead"], createdBy: { source: "USER", channel: "COPY" } });
    // Matching opted-in MASTER contact on the same phone. Backlog migration now
    // re-verifies attribution evidence against the LIVE master (it no longer
    // trusts a stored registry status), and the strict gate requires populated
    // marketing attribution.
    mockContact({ id: `master_pol_${phone.slice(-4)}`, locationId: "loc_master", firstName: "PolMaster", phone, attributionSource: { utmSource: "fb", medium: "paid" } });
  }
  // migrateTagMode fixtures: broker contacts carrying TWO of their OWN tags plus a
  // matching opted-in master, to exercise keeping all / only-listed / none of the
  // old broker tags on a backlog migration.
  for (const [id, phone] of [
    ["broker_migtag_none", "+15550912001"],
    ["broker_migtag_list", "+15550912002"],
  ]) {
    mockContact({ id, locationId: "loc_broker_a", firstName: "MigTag", phone, email: `${id}@x.com`, tags: ["fb lead", "vip"], createdBy: { source: "USER", channel: "COPY" } });
    mockContact({ id: `master_migtag_${phone.slice(-4)}`, locationId: "loc_master", firstName: "MigTagMaster", phone, attributionSource: { utmSource: "fb", medium: "paid" } });
  }
}
...
/**
 * Add tags to a contact via the dedicated endpoint. Unlike tags set in the create
 * body, this emits a "tag added" event, which is what fires GHL's tag-triggered
 * workflows in the broker sub-account. Idempotent (re-adding an existing tag is a
 * no-op on GHL's side).
 */
async function addTags(contactId, tags, token) {
  ...
  if (isMock()) {
    ...
    const c = mockDb.contacts.get(contactId);
    if (!c) throw new GhlError("Contact not found (mock)", 404, null);
    const have = new Set((c.tags || []).map((t) => String(t).toLowerCase()));
    c.tags = [...(c.tags || [])];
    for (const t of list) if (!have.has(t.toLowerCase())) c.tags.push(t);
    return { tags: c.tags };
  }
  const data = await ghlFetch(`/contacts/${encodeURIComponent(contactId)}/tags`, {
    token,
    method: "POST",
    body: { tags: list },
  });
  return data ?? { tags: list };
}
