...
function seedMocks() {
  ...
  for (const [id, phone] of [
    ...
    // registry status), so the master must carry genuine evidence.
    mockContact({ id: `master_pol_${phone.slice(-4)}`, locationId: "loc_master", firstName: "PolMaster", phone, createdBy: { source: "INTEGRATION" } });
  }
}
