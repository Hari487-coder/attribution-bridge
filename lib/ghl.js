...
async function ghlFetch(path, { token, method = "GET", body, query, retryOn429 = 4 } = {}) {
  ...
  for (let attempt = 0; ; attempt++) {
    ...
    // GHL rate limit — back off and retry (respect Retry-After when present).
    if (res.status === 429 && attempt < retryOn429) {
      const ra = Number(res.headers.get("retry-after"));
      const waitMs = Number.isFinite(ra) && ra > 0 ? ra * 1000 : 800 * 2 ** attempt;
      await sleep(waitMs);
      continue;
    }
    break;
