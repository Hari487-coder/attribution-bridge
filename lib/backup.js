...
// Per-tenant paths, resolved per-request via tenant.registryPath()/backupDir().
const KEEP = 14;
...
function buildBundle(at) {
  return {
    ...
    config: loadConfig(),
    registry: readJson(tenant.registryPath(), {}),
  };
}
...
function restoreBundle(bundle) {
  ...
  const currentReg = readJson(tenant.registryPath(), {});
  // Never let a restore rotate an ESTABLISHED workspace's HMAC signing secret.
  // That keeps the verification-marker signature an independent control: a crafted
  // bundle cannot supply a secret under which its own forged "active" registry
  // entries would validate. A fresh/disaster-recovery instance (no secret yet)
  // still adopts the bundle's secret so a legitimately signed registry restores.
  const currentSecret = loadConfig().signingSecret;
  saveConfig(currentSecret ? { ...bundle.config, signingSecret: currentSecret } : bundle.config);
  if (bundle.registry && typeof bundle.registry === "object") {
    ...
    atomicWrite(tenant.registryPath(), JSON.stringify(merged, null, 2));
  }
...
function writeSnapshot(tag) {
  try {
    fs.mkdirSync(tenant.backupDir(), { recursive: true });
    const stamp = new Date().toISOString().replace(/[:.]/g, "-");
    const name = `backup-${stamp}${tag ? "-" + tag : ""}.json`;
    atomicWrite(path.join(tenant.backupDir(), name), JSON.stringify(buildBundle(), null, 2));
    const files = fs
      .readdirSync(tenant.backupDir())
      .filter((f) => f.startsWith("backup-") && f.endsWith(".json"))
      .sort();
    for (const f of files.slice(0, Math.max(0, files.length - KEEP))) {
      try {
        fs.unlinkSync(path.join(tenant.backupDir(), f));
      } catch {
        /* ignore prune errors */
      }
    }
